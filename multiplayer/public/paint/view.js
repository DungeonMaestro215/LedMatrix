...
class View {
    ...
    setup() {
        document.getElementById('clear').addEventListener('click', () => this.updateListeners({ type:'clear' }));
        document.getElementById('test').addEventListener('click', () => this.updateListeners({ type:'test' }));
    ...
    handlePainting(e) {
        ...
        if (e.touches) {
            ...
            this.updateListeners({ type: 'color', cell_num: cell_num, color: color, size: size });
            return;
        }
        ...
        if (button == 2) {
            const color = document.getElementById(`colorpicker${button}`).value;
            this.updateListeners({ type: 'fill', cell_num: cell_num, color: color });
        }
    }
}